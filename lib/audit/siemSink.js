...
// V15 (item #11) — bump this counter on every SIEM emit failure so
// Grafana/Wazuh can alert on `rate(siem_emit_failures_total[5m]) > 0`.
//
// prom-client transitively requires('fs'), which webpack tries to bundle
// for the Edge runtime when it follows the
//   instrumentation.js → siemSink.js → prometheus.js → prom-client → fs
// chain. The same eval('require') escape hatch we use for fs/path below
// keeps the prometheus module out of the Edge bundle. Safe because:
//   - lib/prometheus.js is a hard-coded constant string
//   - siemSink itself is only reached from Node-runtime code paths
let _failuresCounter = null
function bumpFailure(reason) {
    try {
        if (!_failuresCounter) {
            const nodeRequire = eval('require')
            const mod = nodeRequire('../prometheus.js')
            _failuresCounter = mod.siemEmitFailures
        }
        _failuresCounter.inc({ reason })
    } catch {
        // metric infrastructure unavailable — swallow, the throttled
        // console.error has already logged the underlying failure.
    }
}
...
export async function initSiemSink() {
    ...
    // Use eval('require') to load Node built-ins. This is the canonical
    // escape hatch for bundled-by-webpack code that must reach Node's
    // native module loader. Plain `import` / `require` / `await import`
    // all get rewritten by webpack and can fail in the Edge-runtime bundle
    // (Next.js compiles instrumentation.js for BOTH runtimes). eval()
    // hides the call from webpack's static analyzer entirely.
    //
    // Safe because:
    //   - The strings 'fs' and 'path' are hard-coded constants, not user input.
    //   - This code path only runs when SIEM_AUDIT_PATH is set AND
    //     NEXT_RUNTIME === 'nodejs' (gated upstream in instrumentation.js).
    //
    const nodeRequire = eval('require')
