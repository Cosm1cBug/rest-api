 ...
 * @param {{
 *   label?:     string,
 *   scopes?:    string[],          // empty / undefined => full access
...
export async function issueApiKey(userId, opts = {}) {
    ...
    // Normalise expiresAt to a Date or null. Callers pass an ISO string
    // from JSON; pass-through if it's already a Date; coerce string -> Date.
    let expiresAt = null
    if (opts.expiresAt) {
        const d = opts.expiresAt instanceof Date
            ? opts.expiresAt
